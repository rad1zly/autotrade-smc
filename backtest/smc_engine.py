"""PAF-QIE backtest engine — structure-shift (MSS) dengan otak LLM.

Alur (sama dengan EA MQL5 live, supaya backtest berarti):
  1. STRUCTURE : swing high/low di-track terus-menerus (fractal-n). Bias
                 (bullish/bearish) ditentukan oleh urutan swing yang confirmed,
                 dan level referensi (swing berlawanan paling baru) itu yang
                 "dijaga" — bukan sweep pool eksternal.
  2. MSS       : close menembus level referensi berlawanan -> bias flip.
                 Ini SATU-SATUNYA syarat trigger (tidak perlu sweep pool dulu).
  3. BRAIN     : begitu MSS confirmed, konteks pasar dikirim ke brain.decision
                 (LLM sungguhan atau mock rule-based) — brain yang menentukan
                 BUY/SELL/SKIP + SL/TP, BUKAN rumus tetap. SL divalidasi harus
...
from common import Trade, resample, atr_map, compute_structure
...
def run_smc(m15: pd.DataFrame, cfg: SmcConfig, decide_fn: Callable[[dict], dict]) -> SmcEngine:
    ...
    for m in range(len(times) - 1):  # -1: entry butuh bar m+1 (open, no lookahead)
        ...
        if event is not None and m - last_close_m < cfg.cooldown_bars:
            ...
            ctx = {
                ...
                "sweep": {"origin": "SWING-H" if bullish else "SWING-L",
                          "side": "HIGH" if bullish else "LOW",
                          "extreme": float(level), "poolPrice": float(level),
                          "time": str(t)},
                "mss": {"dir": "BULLISH" if bullish else "BEARISH",
                        "level": float(level), "time": str(t)},
                "fvg": ({"valid": True, "bullish": bullish,
            ...
            log_row = {
                "mss_time": t, "bias": ctx["bias"], "broken_level": round(float(level), cfg.digits),
            ...
            if not result.get("valid"):
                ...
                entry = float(O[m + 1]) + (cfg.spread if bullish else 0.0)
                ...
                if risk <= 0:
                    ...
                    eng.open_trade = Trade(
                        zone_kind=("SWING-H" if bullish else "SWING-L") + "-mss",
                        zone_t0=t,
                        zone_top=result["tp"], zone_bottom=result["sl"],
                        direction=1 if bullish else -1, entry_time=times[m + 1], entry=entry,
